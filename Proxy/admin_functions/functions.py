...
2) Use the UploadTrainingData providing the path to the folder that stores the training data to upload images and get the corresponding face tokens.
3) Use the addFacesToFaceset function to add all faces to the facesets of the corresponding courses. If a faceset doesn't exist, the api calls will be unsuccessful and you will get "Somethign went wrong" even though you have a good internet connection.

"""

# To create facesets for courses that do not already have a faceset. Implemented by Srinidhi
...
# Assumption is that the images will be stored in the TrainingData folder and the files will be named <roll_no.>.jpg. path is the path to the training data folder. Implemented by Srinidhi
...
# Add all face tokens for the roll numbers present in a course to the facesets of every course present in the Course tables. Implemented by Srinidhi
...
# assuming that student, course and prof are present in the database. Implemented by Pavan
...
# assuming that prof is present in the database. Implemented by Pavan
...
# Adds all CS14 batch students to a course. Implemented by Srinidhi
...
# Creates a single user. Implemented by Srinidhi
...
# Creates user accounts for all CS14 batch students. Implemented by Srinidhi			
def createCS14Users():
	for i in range(1,63):
		if i < 10:
